---
name: install
description: Install Nano CMS (rickfalaschi/nano-cms) into the current directory FROM SCRATCH. Downloads the latest released version as a tarball, gathers DB and admin credentials interactively, writes `.env`, runs the bundled installer (migrations + initial user + per-project files), and records the installed version in `.nano-version` so future upgrades work without depending on git. Use this skill whenever the user wants a fresh install — phrases like "instala o nano aqui", "set up nano-cms in this folder", "bootstrap nano", "create a new nano project", "i just want a working nano-cms here", "instalar o nano nessa pasta". Triggers in Portuguese and English. Do NOT use when: (a) converting static HTML/PHP into a theme — that's nano-cms:theme-convert, (b) updating/upgrading an existing Nano install to a newer version, (c) running specific CLI commands like `bin/nano migrate` or `page:sync` on a project that's already installed, (d) answering reference questions about Nano's schema, helpers, fields, options, or features. Mentioning Nano-specific keywords like "nano", "site.json", or "field()" alone does NOT trigger this skill — only explicit setup/install/bootstrap intent does.
---

# Install Nano CMS

This skill installs Nano CMS into the current working directory. It's an interactive flow: download a versioned release tarball, gather config, configure, run installer, write `.nano-version`, report back. **The project is left without a `.git/` of Nano** — Nano is a dependency, not a fork. Future upgrades use the tarball+manifest model (see `nano-cms:upgrade`).

## Goal of this skill

By the end, the user has:
- A working Nano installation at the current directory (`./core/`, `./bin/`, `./index.php`, etc.)
...
- **No `.git/` of Nano** in the project. Nano is a versioned dependency, not a fork in their codebase.
...
2. Fetch       — download release tarball from rickfalaschi/nano-cms
...
- **If `core/Bootstrap.php` exists** → Nano is already installed here. STOP. Tell the user: "Nano appears to already be installed in this folder. Re-running install would overwrite files. Do you want to (a) abort, (b) wipe and reinstall, (c) run `nano-cms:upgrade` to bring it to the latest version, (d) just run `./bin/nano install` to apply pending migrations on the existing install?" Wait for explicit confirmation.

- **If folder is empty** → ideal case, proceed. The tarball will extract straight into `./`.

- **If folder has non-Nano files** (user's own project, an existing git repo of a different project, random files) → list them and ask: "This folder contains [files...]. Installing Nano here will add Nano's files alongside. Existing files won't be overwritten unless they collide with Nano's tracked files (rare). Continue?" If they confirm, the tarball extracts into the same directory; collisions overwrite (warn the user explicitly about which files would collide before extracting).

- **If `.env` already exists** → ask before overwriting; offer to keep existing values.

## Step 2 — Fetch tarball

Resolve the version to install. Default: latest tagged release on `rickfalaschi/nano-cms`. Override via `--version=v1.2.3` if the user wants a specific version.

```bash
REPO="rickfalaschi/nano-cms"     # default; override with --repo if user passed one
...
EXTRACTED=$(find "$TMPDIR" -maxdepth 1 -type d -name "nano-cms-*" | head -1)
...
echo "Fetched Nano ${TAG}"
```

Why tarball and not git clone:
- **No persistent `.git/` left in the project.** Nano is a versioned dependency — having its git history in your project is conceptually backwards (you don't keep Composer's git in your PHP project either).
...
Leave empty for dev/local — Nano will auto-detect from the request host.
...
- **"Migrations: N applied"** → schema set up. If "MySQL version too old" or similar JSON-column errors appear, report: "Nano requires MySQL 5.7+ or MariaDB 10.2+ (needs native JSON columns)."
...
# Nginx template — Nano's installer only generates Apache config
# (.htaccess), not nginx server blocks. Nano is officially Apache-only.
...
- **Don't remove `migrations/`.** The applied schema lives in the DB, but the migration FILES still need to stay around so deploys (`nano-cms:ssh-deploy update-cms`) can ship new migrations to remote servers. Removing them locally would leave the project unable to push schema changes to prod when Nano upstream releases new migrations later.
...
Earlier versions of this skill removed `migrations/` after install on the theory that "the schema lives in the DB now." That backfired in the deploy flow: `nano-cms:ssh-deploy update-cms` rsyncs `core/`, `bin/`, AND `migrations/` to the server, then runs `bin/nano migrate` remotely to apply pending schema changes. Without local migration files, deploys would silently push core changes that depend on schema that never arrives. Worse, with `rsync --delete`, an empty local `migrations/` could wipe the migrations the server already has.
...
After cleanup succeeds, record what version was just installed. This is the file that future `nano-cms:upgrade` reads to know "where you are" — it's how the project tracks its Nano version once `.git/` is gone.
...
SOURCE_REPO="${REPO:-rickfalaschi/nano-cms}"
...
The `manifest_url` is a convenience: `nano-cms:upgrade` can fetch it without re-deriving the URL pattern. Updates to that pattern get recorded with the install — future-proof.

`.nano-version` lives at the project root and **goes along with the project** when deployed via `nano-cms:ssh-deploy` or downloaded via `nano-cms:ssh-download`. So any machine that has the project knows what version it's running. (If the user runs `git init` later, they'll likely want to commit this file — it's part of the project's identity, not a secret.)
...
Next steps:
  ...
  - Drop a theme into ./theme/ (or use the nano-cms:theme-convert skill to build one
...
- **Tarball download fails (network, 404, etc.)** → show the curl stderr verbatim, abort. Don't leave partial state. If the user passed `--version=` with a non-existent tag, suggest checking https://github.com/rickfalaschi/nano-cms/tags for valid tags.
...
- Theme creation/conversion → that's `nano-cms:theme-convert`
