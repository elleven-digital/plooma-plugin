...
# Convert static templates into a Plooma theme

This skill turns a folder of static HTML/PHP pages into a working Plooma theme. It analyzes the source, proposes a schema, gets user approval, then generates the theme.
...
- `theme/templates/page-*.php` and `theme/templates/single-*.php` generated with Plooma helpers wired in (`field()`, `option()`, `image_url()`, `get_header()`, `get_footer()`)
...
- `./bin/plooma page:sync` runs successfully and `./bin/plooma schema:validate` reports OK
...
**Scenario A — Inside an existing Plooma's `theme/` folder.**
Detect by checking if `../core/Bootstrap.php` exists. If yes, this is the destination — generate files in place (current dir).

**Scenario B — Folder of templates outside any Plooma.**
Detect by checking for absence of `core/Bootstrap.php` (here or one level up). In this case:
1. Tell the user: "Detected this folder isn't inside a Plooma install. Plan: create `./theme/`, move all HTML/PHP/asset files into it, install Plooma in current dir, then convert. OK?"
2. On confirmation:
   - `mkdir theme`
   - Move all source files (HTML/PHP/CSS/JS/images, but NOT `.git`, `.github`, or any dotfiles) into `theme/`
   - Invoke the **plooma:install** skill to set up Plooma in the current dir (it'll clone into root, write `.env`, run installer)
...
- Logo → still hardcoded SVG/img in `partials/header.php` (per Plooma's convention — site logo is theme-level, not editable per project unless requested)
...
- **Image references** — capture the relative path as found (e.g., `assets/cases/wm-educacao.jpg`). Don't try to upload to the media library; the seeder will write the path string and Plooma's `image_url()` resolves bare paths so the page renders immediately.
...
- Page `url` field: `/` for home, `/<key>` otherwise (omit if matches the default — Plooma falls back to `/<key>`)
...
- DON'T add a built-in SEO field group — Plooma handles SEO automatically for paged content (`Config::seoFields()` is built-in)

### 3.2. Write `partials/header.php` and `partials/footer.php`

Each follows Plooma conventions:
...
```php
<?php /** @var \Plooma\TemplateContext $page */ ?>
...
Conventions to follow strictly (these are why Plooma works):
...
```php
<?php /** @var \Plooma\TemplateContext $item */ ?>
...
The archive page (e.g. `page-blog.php`) uses `items('post')` to loop. Plooma doesn't auto-route `/<type-slug>` archives anymore — each archive must be a configured page in `site.json`. Make sure the plan's `pages.blog` template loops items.
...
**Inspect Plooma's models before writing the seed.** Don't guess the API. Read these files to learn the exact write surface:
...
If you can't confidently find the option write API after a few minutes of reading, **stop and tell the user**: "I can't find how options are written in this Plooma version. I'll seed pages, items, and terms — you'll fill in options manually via admin." Don't ship a broken seed.

**Where to put it.** Write `theme/install/seed.php`. It's a one-shot script bootstrapped against Plooma's core:
...
use Plooma\Models\Page;
use Plooma\Models\Item;
use Plooma\Models\Term;
...
**Image fields → path strings, not media uploads.** Set them to the original relative path (e.g. `theme/assets/cases/wm-educacao.jpg`). Plooma's `image_url()` accepts path strings as input, so the page renders with the original images immediately. The user can later upload via the media library and the integer ID replaces the string. **Don't try to programmatically upload to the media library** — too brittle, too easy to corrupt, and it's not what the user asked for.
...
```bash
./bin/plooma schema:validate
./bin/plooma page:sync
...
- **Model API mismatch** — your inspection of Plooma's source produced wrong assumptions. Re-read the model file, fix the call, re-run. The script's idempotency guard means re-running is safe (it'll skip if home is already populated, otherwise pick up where it left off).
...
**Try it:**
- `./bin/plooma serve 8080` (if dev), or open the configured domain
...
When ambiguous, lean toward **editable** — Plooma's admin handles empty fields gracefully (Plooma-style templates use `<?php if ($field !== ''): ?>` patterns), and giving the user one extra field to ignore is better than baking content into PHP they can't reach from the admin.
