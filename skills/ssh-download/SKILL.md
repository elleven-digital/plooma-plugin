...
.deploy/ssh.json exists in cwd? ──no──▶ Mode A: first-time download
      ...
      yes
       │
       ▼
core/Bootstrap.php exists?  ──no──▶ Mode A: first-time (config existed but no Plooma —
...
**Mode A (first-time)**: gather everything from the user, validate, save config, download. Creates a working Plooma install in the current folder.
...
3. If folder is non-empty but has no `.deploy/ssh.json` and no `core/Bootstrap.php`, this is ambiguous — STOP and ask the user: "Não detectei nem Plooma nem `.deploy/ssh.json`, mas a pasta tem arquivos. Quer (a) abortar, (b) tratar como nova instalação (vou backupear esses arquivos antes de baixar)?"
...
**Remote path** — `ssh.remote_path` is the absolute path on the server where Plooma lives. Same hosts table as plooma:ssh-deploy:
...
After gathering, save to `.deploy/ssh.json`, `chmod 0600`. Same schema as `plooma:ssh-deploy` — this skill and that one share the config file.
...
2. **Remote has Plooma** — `ssh <opts> "test -f <remote_path>/core/Bootstrap.php && echo HAS_NANO || echo NO_NANO"`. If NO_NANO, abort with: "Não encontrei Plooma em `<remote_path>` no servidor. O caminho está correto?"
...
5. **Local DB user has DROP/CREATE privileges** — try `mysql -e "CREATE DATABASE IF NOT EXISTS __plooma_priv_check; DROP DATABASE __plooma_priv_check"`. If permission denied, abort.
...
**Mode B (existing Plooma):**
```
⚠️  Refresh plan (DESTRUCTIVE):

  This folder currently has a Plooma install. Refreshing will:
    • Backup local files → /tmp/plooma-download-backup-<ts>/files/
    • Backup local DB → /tmp/plooma-download-backup-<ts>/local-db.sql
...
1. **Backup (Mode B only)**: 
   - `mkdir -p /tmp/plooma-download-backup-<ts>/files`
   ...
   - Dump local DB to `/tmp/plooma-download-backup-<ts>/local-db.sql`
...
1. Run `php bin/plooma migrate:status` from current dir. Should report "All migrations are up to date" (the dump included the migrations table state).
2. Optional: print the path to `.env` and remind user to test with `./bin/plooma serve 8080` or their local web server.
...
- Uses `--delete` on rsync without verifying remote has Plooma (preflight covers this).
...
- **PHP not in PATH**: needed for the `php bin/plooma migrate:status` verify step. Less critical here than in deploy. If not found, skip verification with a warning.
...
| `preflight.sh` | All 5 preflight checks (SSH + remote Plooma + remote DB + local DB + local DB privs) |
...
- Deploy TO a server (use `plooma:ssh-deploy`)
- Install Plooma without a remote source (use `plooma:install`)
