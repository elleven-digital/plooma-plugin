...
A Plooma deploy has two "halves" that change at different cadences:
...
- **Engine** lives in `core/`, `bin/`, `migrations/`, plus root files (`index.php`, `.htaccess.example`, `robots.txt.example`). Changes when Plooma core is upgraded or new migrations land. Touching it requires running `migrate`. Plooma uses a flat layout — `index.php` is at the project root, not inside a `public/` subdir.
...
|---|---|---|
| `init` | Everything: files + DB import + storage + .env + robots.txt | Plooma already exists at target |
| `update-cms` | `core/`, `bin/`, `migrations/` + root engine files (`index.php`, `.htaccess.example`, `robots.txt.example`). Runs `migrate` after | Plooma does NOT exist at target |
| `update-theme` | `theme/` only (excluding `theme/install/`, `storage/uploads/`, `.env`). Runs `schema:validate` + `page:sync` after | Plooma does NOT exist at target |
...
target state ── EMPTY ──▶ offer init (Phase 3)
                          (or update-* if user explicitly asks
                           and explains they expect Plooma elsewhere)
       │
      EXISTS
       │
       ▼
offer update-cms / update-theme. Refuse init.
Tell user clearly: "Plooma já existe em <path>.
...
**Remote path** — `ssh.remote_path` is the absolute path on the server where Plooma will live. This is THE most important field, and it varies by host:
...
- `site.subpath` — only set if the site lives under a subpath, e.g. `/blog`. **This affects Plooma's `APP_BASE_PATH` env var** — the skill writes it to `.env` automatically. Empty string means root domain.
...
**Database** (must be already created on the remote — Plooma can't create databases, the user does it via host panel or `mysql` CLI):
...
| "atualiza o cms", "push do core", "rodar migrations em prod", "sobe atualização do plooma" | `update-cms` |
...
1. Re-checks target is empty (refuses if Plooma exists)
...
6. Runs on remote: `php bin/plooma migrate` (just in case the dump didn't include the migration log table state)
...
1. Verifies Plooma exists at target (refuses if EMPTY)
2. `rsync -avz` (with per-subtree `--delete`) of `core/`, `bin/`, `migrations/` + Plooma-shipped root files (`index.php`, `.htaccess.example`, `robots.txt.example`) — **not** theme, storage, .env, live `.htaccess`, live `robots.txt`
3. Runs on remote: `php bin/plooma migrate` to apply any pending migrations from the new files
...
2. `rsync -avz` (no `--delete`) of `theme/` only, with **excludes**:
   ...
   - `.env`
3. Runs on remote: `php bin/plooma schema:validate && php bin/plooma page:sync`
...
- Runs `init` against an existing Plooma install
...
| `init.sh` | Full init deploy. Refuses if target has Plooma. |
...
- Skip the target inspection (Phase 0). Even if the user is sure Plooma isn't there, check. The cost is one SSH command; the benefit is preventing a wipe.
